...
resource "google_compute_region_backend_service" "default" {
  project                         = var.project
  name                            = var.name # var.health_check["type"] == "tcp" ? "${var.name}-with-tcp-hc" : "${var.name}-with-http-hc"
  region                          = var.region
  network                         = var.network
...
resource "google_compute_health_check" "tcp" {
  ...
  tcp_health_check {
    ...
    request      = var.health_check["request_path"]
...
resource "google_compute_firewall" "default-hc" {
  ...
  allow {
    protocol = "tcp"
    ports    = [var.health_check["port"], "443"]
  }
