 ...
 * distributed under t    cmek_list = [
    {
      project_id           = "prj-shared-common-srv46"
        location           = "europe-west3"
        keyring            = "kr-common-srv-bshg-lz-01"
        keys               = ["key-common-srv-intranet-facing-lz-01", "key-common-srv-internet-facing-lz-01"]
        labels = {
      "customer"     : "bshg",
      "requestor"    : "ankireddy-akhil-bshg-com",
      "businessunit" : "common",
      "costcenter"   : "1111",
      "environment"  : "shared",
      "appname"      : "landing-zone",
      "project"      : "prj-shared-common-srv",
      "ticketid"     : "2",
      "owner"        : "lalit-sharma-bshg-com"
    }
}
]
...
resource "google_kms_crypto_key_iam_binding" "users" {
  count         = length(var.set_users_for)
  role          = "roles/cloudkms.cryptoKeyEncrypterDecrypter"
  crypto_key_id = local.keys_by_name[var.set_users_for[count.index]]
  members       = compact(split(",", var.users[count.index]))
}
