...
#added
resource "google_compute_global_forwarding_rule" "http-test" {
  for_each   = { for name in var.frontends : name.name => name }
  project    = var.project
  name       = "${var.name}-${each.value.name}-http"
  target     = google_compute_target_http_proxy.default-test[each.value.name].self_link
...
#added 1
resource "google_compute_global_forwarding_rule" "https-test" {
  for_each = { for name in var.frontends : name.name => name }
  project  = var.project
  #count      = var.ssl ? 1 : 0
  name       = "${var.name}-${each.value.name}-https"
  target     = google_compute_target_https_proxy.default-test[each.value.name].self_link
  ip_address = each.value.address
  port_range = "443"
}
...
# HTTP proxy when http forwarding is true
#added
resource "google_compute_target_http_proxy" "default-test" {
  for_each = { for name in var.frontends : name.name => name }
  project  = var.project
  name     = "${var.name}-${each.value.name}-http-proxy"
  url_map  = var.https_redirect == false ? local.url_map : join("", google_compute_url_map.https_redirect.*.self_link)
}
...
#HTTPS proxy when ssl is true
#added 2
resource "google_compute_target_https_proxy" "default-test" {
  for_each = { for name in var.frontends : name.name => name }
  project  = var.project
  #count   = var.ssl ? 1 : 0
  name    = "${var.name}-${each.value.name}-https-proxy"
  url_map = local.url_map

  ssl_certificates = compact(concat(each.value.ssl_certificates, google_compute_ssl_certificate.default.*.self_link, google_compute_managed_ssl_certificate.default.*.self_link, ), )
  ssl_policy       = each.value.custom_ssl_policy
  quic_override    = var.quic ? "ENABLE" : null
}
...
resource "google_compute_backend_service" "default" {
  ...
  security_policy = module.cloud_armor[each.value.security_policy].policy
  dynamic "backend" {
...
resource "google_compute_health_check" "default" {
  ...
  dynamic "http_health_check" {
    for_each = lookup(each.value["health_check"], "protocol", "HTTP") == "HTTP" ? [
      {
  ...
  dynamic "https_health_check" {
    for_each = lookup(each.value["health_check"], "protocol", "HTTP") == "HTTPS" ? [
      {
        host         = lookup(each.value["health_check"], "host", null)
        request_path = lookup(each.value["health_check"], "request_path", null)
        port         = lookup(each.value["health_check"], "port", null)
  ...
  dynamic "http2_health_check" {
    for_each = lookup(each.value["health_check"], "protocol", "HTTP") == "HTTP2" ? [
      {
        host         = lookup(each.value["health_check"], "host", null)
        request_path = lookup(each.value["health_check"], "request_path", null)
        port         = lookup(each.value["health_check"], "port", null)
...
resource "google_compute_firewall" "default-hc" {
  ...
  for_each = var.backends
  project  = var.project
  name     = "${each.value.backend}-hc"
  network  = each.value.firewall_networks
  source_ranges = [
    "130.211.0.0/22",
    "35.191.0.0/16"
  ]
  ...
  dynamic "allow" {
    ...
    content {
      protocol = "tcp"
      ports    = [each.value.health_check.port, "443"]
    }
  }
}
...
module "cloud_armor" {
  source            = "./cloud_armor"
  for_each          = { for x in var.list_of_cloud_armor : x.policy_name => x }
  policy_name       = each.value.policy_name
  project           = var.project
  type              = each.value.type
  armor_description = each.value.armor_description
